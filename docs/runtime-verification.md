...
The [judge guide](submission/judge-testing.md) contains the standalone APK metadata/checksum and access instructions. The APK includes JavaScript and runs without Metro. The [APK download](https://github.com/kcyoow/quietpilot-agents-for-humans/releases/download/v0.1.0-hackathon/quietpilot-preview.apk) is public; arrange Google access through the submission contact: OAuth testing currently admits two allowlisted users. The [preparation status](submission/preparation-status.md) tracks the remaining handoff.
...
| Fresh account | Signup/sign-in and Gmail consent/return succeeded. The initial list did not show the original account's work. Session and connection survived replacement with an APK signed by the same dedicated signer; notification ON persisted. | Further fresh-account Calendar/push checks were intentionally skipped and remain unverified. This observed separation is not a comprehensive multi-tenant security audit. |
| Standalone APK | ARM64 version 0.1.0/code 1, min API 24/target 36, 46,371,800 bytes, non-debuggable, dedicated signer, 4,477,512-byte embedded JavaScript bundle. The fresh-account path ran without Metro. | Verified on a fresh Android emulator. Physical devices, other ABIs and iOS are not verified; the APK is public. Full SHA-256 is in the judge guide. |
...
- The standalone English APK and fresh-account signup/sign-in/Gmail consent/persistence checks passed. Further fresh-account Calendar/push checks were intentionally skipped and remain unverified; the APK is publicly available. The completed 62-message scan still has an optional-action preparation warning. Existing source-derived text and historical results are preserved; English captions may translate their meaning without rewriting them.
